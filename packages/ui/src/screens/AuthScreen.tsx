...
export function AuthScreen({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true);
    try {
      if (mode === 'signup') await client.auth.signup({ name: name.trim(), email: email.trim(), password });
      else await client.auth.login({ email: email.trim(), password });
      navigate('app');
    } catch (error) {
      if (error instanceof NetworkError)
        setFormError('You’re offline. Noted keeps working, so log in once you’re back online.');
